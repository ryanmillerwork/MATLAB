...
for s=1:size(subjects,1) %For each subject
    ...
    for i=1:size(trials,1) %For each trial
        ...
        if double(cell2mat(fetch(DataDB, ['Select SubjectBlock FROM trialsTable WHERE UniqueTrial = ' num2str(thisTrial)]))) ~= 1
            continue;
        end
...
%         graspMatBig(:,:,end+1)=graspMat;
        ...
        if c < -0.11 & c > -0.13
            figure; subplot(2,1,1); hold on; title('Eye position'); axis tight
            imagesc(eyeMat)
            subplot(2,1,2); hold on; title('Hand position'); axis tight
            imagesc(graspMat)
        end
