...
wait=300; %milliseconds to wait because of initial burst response
...
for listI=1:size(list,1) %For each neuron in the list
    ...
    for i=1:size(stimOrder,1) %For each trial, grab spike times and spike codes
        ...
        trial(counter,1).spikes=times(sorts==sort)-stimStart;
...
%     d3=double(pileR2)-double(pileR3);
    ...
    savestring=['filename' 'chan' num2str(chan) 'sort' num2str(sort)];
...
%     pileR3=uint16(pileR3);
    ...
    imwrite(pile,[savestring '.png'])

%     imwrite(pileR3,[savestring 'Ref3.png'])
