...
% Tone frequency and intensity
toneOptions=logspace(2,4.2);                            %List of possible frequencies
toneHz=toneOptions(randperm(length(toneOptions),1));    %Choose one at random
toneScale=.4;%rand(1);

% How many times to we wish to play the sound (each time)
repetitions = 1;
...
myBeep = MakeBeep(toneHz, beepLengthSecs, freq);
myBeep = myBeep*toneScale + randn(size(myBeep));
