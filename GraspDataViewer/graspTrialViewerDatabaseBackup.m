...
conn = sqlite('C:\Users\Ryan\Documents\Data\graspHuman.db');
plotFigure=gcf;
...
if iscell(choice1Shape)
    ...
    gTimesL=cell2mat(fetch(conn,['SELECT Time_1 FROM graspTable' options.trialID ' WHERE TrialPeriod = 1']));
...
if eyeSample <= length(eyePos(:,1))
    ...
    sacTimes=find(eyePosSac(1:eyeSample));
    blinkTimes=find(eyePosBlink(1:eyeSample));
