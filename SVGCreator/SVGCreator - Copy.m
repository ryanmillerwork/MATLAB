...
saveString=[chosen{1} '_' num2str(options.unitsPerMM) '_' num2str(options.ratio) '_' num2str(options.contacts) '_' num2str(options.slotDepth/options.unitsPerMM) '_' num2str(options.slotWidth) '_' num2str(options.symmetric) '_' num2str(options.radio) '_' num2str(options.left) '.svg'];
...
global options points  slotSpots
...
newmp=mp-first+1;
% slotSpots(1)=1; slotSpots(newmp)=1;
...
plotFinal(newxs,newys)
plot([0 0],[-1 1],':')

function plotFinal(xs,ys)
hold on;
axis equal
xlim([-0.75 0.75])
ylim([-0.75 0.75])
...
%Get filename from box and set axes
